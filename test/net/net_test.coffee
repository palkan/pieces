...
describe "Net", ->
  ...
  describe "XHR", ->
    ...
    it "prepare params with Date and File/Blob", ->
      d = new Date()
      data = 
        item:
          tags: ['1','2']
    ...
    it "create query from data", ->
      data = id:1, name:'Ivan Fuckov'
      expect(net._data_to_query(data)).to.eq 'id=1&name=Ivan%20Fuckov' 

    it "send get request without data", (done) ->
      net.get('/echo?q=1').then( (data) ->
        expect(data.q).to.eq '1'
        done()
      ).catch(done)

    it "send get request with data", (done) ->
      net.get('/echo',{q:1}).then( (data) ->
        expect(data.q).to.eq '1'
        done()
      ).catch(done)

    it "send get request with data and url query", (done) ->
    ...
    it "send get request with nested data", (done) ->
    ...
    it "send post request with data", (done) ->
    ...
    unless window.mochaPhantomJS
      it "send patch request with data", (done) ->
      ...
      it "send delete request with data", (done) ->
    ...
    it "send post request with data as json", (done) ->
