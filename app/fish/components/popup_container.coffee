...
class pi.PopupContainer extends pi.Base
  postinitialize: ->
    ...
    @listen ".#{pi.klass.POPUP_OVERLAY}", 'click', (e) => @handle_close()

  add_overlay: ->
    @overlay = pi.Nod.create('div').piecify()
    @overlay.addClass pi.klass.POPUP_OVERLAY
    @overlay.hide()
  ...
  add_container: ->
    ...
    @cont.addClass pi.klass.POPUP_CONTAINER
    @overlay.style("z-index",++@z)
  ...
  open: (@target, options = {}) ->
    ...
    @target.addClass pi.klass.POPUP
    @target.hide()
    @cont.append @target
  ...
  setup_target: (target) ->
    ...
    if options.close is false
      @addClass pi.POPUP_NO_CLOSE
    else
      @removeClass pi.POPUP_NO_CLOSE
  ...
  close: ->
    ...
    if @__overlays__.length is 1
      @opened = false
      @trigger pi.Events.Opened, false

    new Promise(
      (resolve) =>
        utils.after @hide_delay, =>
          @target.removeClass pi.klass.POPUP
