...
class pi.Sorters extends pi.Base
  ...
  postinitialize: ->
    ...
    for sorter in @sorters
      ...
      if sorter.hasClass(pi.klass.SORTER_DESC)
        @update_sorter sorter, 'desc'
      else if sorter.hasClass(pi.klass.SORTER_ASC)
  ...
  sorter_click: () ->
    @_sorter_click ||= (e) =>
      ...
      @trigger pi.Events.Update, @value()
  ...
  update_sorter: (sorter, new_state) ->
    ...
    sorter.removeClass pi.klass.SORTER_DESC, pi.klass.SORTER_ASC
    new_klass = if new_state is 'desc' then pi.klass.SORTER_DESC else pi.klass.SORTER_ASC
    sorter.addClass new_klass
