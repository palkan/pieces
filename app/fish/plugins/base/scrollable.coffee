...
utils.debug 'scroller type', _style_type

class pi.Base.Scrollable extends pi.Plugin
  id: 'scollable'
  initialize: (@pane) ->
    @content = @pane.find(".#{pi.klass.SCROLL_CONTENT}")
    unless @content
      @content = pi.Nod.create(@pane.node.children[0]).addClass(pi.klass.SCROLL_CONTENT)
    
    @create_scroller()
    @hide_native_scroller()

    @always_show = @pane.hasClass pi.klass.HAS_SCROLLER 
  ...
  create_scroller: ->
    @track = Nod.create('div').addClass(pi.klass.SCROLL_TRACK)
    @thumb = Nod.create('div').addClass(pi.klass.SCROLL_THUMB)
    @track.append @thumb
    @pane.append @track
    @pane.addClass pi.klass.SCROLL_PANE
  ...
  thumb_mouse_down: ->
    @__tmd ||= (e) =>
      ...
      @_wait_drag = utils.after 300, =>
        @_startY = e.pageY
        @track.addClass pi.klass.ACTIVE
  ...
  mouse_up_listener: ->
    @__mul ||= =>
      @track.removeClass pi.klass.ACTIVE
