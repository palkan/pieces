import {EventDispatcher} from '../../../src/core/events/dispatcher'
import {EventListener} from '../../../src/core/events/listener'

describe('EventDispatcher', () => {
  var subject, spy, event
  ...
  describe('#on', () => {
    ...
    it('with custom context', () => {
      subject.name = 'a'
      var obj = { name: 'b' }
      var fun = function(){ spy(this.name) }
  ...
  describe('delegators', () => {
    ...
    beforeEach(() => {
      EventDispatcher.register_type_listener('fake', FakeListener)
    })

    afterEach(() => {
      EventDispatcher.unregister_type_listener('fake', FakeListener)
    })
    
    it('create listeners for type', () => {
      subject.on('fake', spy)
      FakeListener.do_fake()
      expect(spy).toHaveBeenCalled()
      subject.off('fake')
      FakeListener.do_fake()
      expect(spy.calls.count()).toBe(1)
